...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        labels_A = torch.empty(0, 3, 256, 256).cuda()
        labels_B = torch.empty(0, 3, 256, 256).cuda()
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            if total_iters % opt.print_freq == 0:    # print training losses and save logging information to the disk
                ...
                evaluator_A.process(model.fake_A.permute(0,2,3,1), model.real_A.permute(0,2,3,1), losses["G_A"])
                predictions_A = torch.cat((predictions_A, model.fake_A), dim=0)
