#[https://github.com/mseitzer/pytorch-fid/blob/master/src/pytorch_fid/fid_score.py#L113]
...
def calculate_fid_given_labels_and_preds(labels, preds, batch_size, device, dims, num_workers=1):
    ...
    m1, s1 = compute_statistics_of_labels_and_preds(
        labels, model, batch_size, dims, device, num_workers
    )
    m2, s2 = compute_statistics_of_labels_and_preds(
        preds, model, batch_size, dims, device, num_workers
    )
...
def compute_statistics_of_labels_and_preds(path, model, batch_size, dims, device, num_workers=1):
    files = split_images(path)
...
def split_images(ndarray):
    # Get the number of images (N), width (W), height (H), and channels (C)
    N, W, H, C = ndarray.shape
    # Initialize an empty list to store the individual images
    images_list = []
    # Iterate over the first dimension (N) of the ndarray
    for i in range(N):
        # Extract the i-th image from the ndarray
        image = ndarray[i]
