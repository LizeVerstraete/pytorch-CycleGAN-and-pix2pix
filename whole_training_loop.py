...
if __name__ == '__main__':
    ...
    visualizer.print_logs("number of training images", dataset_size_train)
    ...
    visualizer.print_logs("number of val images", dataset_size_val)
    ...
    visualizer.print_logs("number of test images", dataset_size_test)
    visualizer.print_logs("options: ",message)
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        opt.aligned = False
        train_loop(dataset_train,total_iters,opt,epoch_iter,model,visualizer,epoch,dataset_size_train,evaluator_train,evaluations_train,iter_data_time, best_loss)
        stop_training,best_epoch = val_loop(evaluator_val,model,dataset_val,opt,total_iters_val,visualizer,epoch,epoch_iter_val,dataset_size_val,iter_data_time,last_fifteen_losses,stop_training, save_dir)
        opt.aligned = True
        test_loop(evaluator_test,evaluations_test,model,dataset_test,opt,save_dir,epoch)
