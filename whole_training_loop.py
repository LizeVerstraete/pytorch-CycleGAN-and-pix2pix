...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        opt.aligned = False
        train_loop(dataset_train,total_iters,opt,epoch_iter,model,visualizer,epoch,dataset_size_train,evaluator_train,evaluations_train,iter_data_time)
        stop_training,best_epoch = val_loop(evaluator_val,evaluations_val,model,dataset_val,opt,total_iters,visualizer,epoch,epoch_iter,dataset_size_val,iter_data_time,last_fifteen_losses,stop_training)
        opt.aligned = True
